...
public class BoardController : MonoBehaviour {
    ...
    IEnumerator GoToAlpha() {
        ...
        isBreathing = (bloodScreen.color.a>=_maxBloodScreen);
    ...
    private void GenerateRandomMap() {
        ...
        if (Random.Range(0, 100) > 100 - (Mathf.Pow(_playerLuck, 2f) * 2)) {
    ...
    void Reload(List<Cell> levelData) {
        ...
        //FillParticlesInMap(_board.Find(c => c.Type == CellType.Entrance));
    ...
    private void FillParticlesInMap(Cell UncoverCell) {

        //particlesPrefab

        List<Cell> positionsOfUncoverCell = GetNeighbours(UncoverCell.Pos.x, UncoverCell.Pos.y);

        foreach (Cell cell in positionsOfUncoverCell) {

            if (CanTouchCell(cell) && (!cell.isRevelated || cell.Type != CellType.Entrance || cell.Enemy != null || cell.Locks != 0)) {

                //particlesContainer.SetTile(particlesContainer.WorldToCell(new Vector3(cell.Pos.x, cell.Pos.y)), particlesPrefab);
                var particles = Instantiate(particlesPrefab, particlesContainer.transform);
                particles.transform.position = new Vector3(cell.Pos.x, cell.Pos.y, 0);
        ...
        bool[,] positionsWithParticles = new bool[boardSizeCol, boardSizeFil];
        ...
        children.ForEach(child => {
            ...
            int[] childTruePos = { Mathf.FloorToInt(child.transform.position.x + 0.5f), Mathf.FloorToInt(child.transform.position.y + 0.5f) };
            var cellToCompare = _board.Find(c => c.Pos.x == childTruePos[0] && c.Pos.y == childTruePos[1]);

            if (cellToCompare.isRevelated || cellToCompare.Locks != 0 || cellToCompare.Type == CellType.Entrance) {

                Destroy(child);

            }

            if (!positionsWithParticles[childTruePos[0], childTruePos[1]-2]) {

                positionsWithParticles[childTruePos[0], childTruePos[1]-2] = true;
    ...
    private void WeaponSkillsSetUpData() {
        ...
        switch(_weaponEquiped) {
            ...
            //Super golpe
                ...
                break;
            case 17:
            //Revisar ya que sera el más random y puede dar problemas / tendra el doble de cooldown
        ...
        if (skillsToPrint[0] != -1) {
            ...
            newPrefabSkill.GetComponent<Skill>().SetSkillData(_weaponEquiped, skillsToPrint[0]);
            newPrefabSkill.GetComponent<Button>().onClick.AddListener(() => SkillTouched());
        ...
        if (skillsToPrint[1] != -1) {
            ...
            var newPrefabSkill = Instantiate(skillPrefab, skillLayer.transform);
            newPrefabSkill.GetComponent<Skill>().SetSkillData(_weaponEquiped, skillsToPrint[1]);
    ...
    void SkillTouched() {
        ...
        Debug.Log("Llego a SkillTouched()");

        switch(_weaponEquiped) {

            case 5:
            //Rayo Solar
                SolarBeam(skillLayer.transform.GetChild(0).GetComponent<Skill>());
                break;
            case 9:
            //3 golpes en cadena night abyss
                break;
            ...
            //Super golpe
                break;
            case 17:
                //Este dejar para el último por hacer
                break;

        }
    }

    void SolarBeam(Skill skill) {

        Debug.Log("Llego a SolarBeam()");

        if (skill.GetCooldownFromSkill() == 0) {

            Debug.Log("Ejecuto esto xd");
            skill.SetMaxCooldownFromSkill();

            var ListOfEnemiesVisible = _board.FindAll(c => c.Enemy != null && c.isRevelated);
            var ListOfEnemiesHidden = _board.FindAll(c => c.Enemy != null && !c.isRevelated);

            foreach (var enemy in ListOfEnemiesVisible) {

                enemy.Enemy.Health = Mathf.Max(0, enemy.Enemy.Health - Mathf.FloorToInt(_playerAttack * 0.75f));
                UpdateUI();
                var ui = GetEnemyUI(enemy.Pos.x, enemy.Pos.y + 2);
                ui.SetData(enemy.Enemy.Attack, enemy.Enemy.Health, enemy.Enemy.EnemySprite, true);
